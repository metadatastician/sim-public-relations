...
# SIM_PUBLIC_RELATIONS ABI/FFI Documentation
...
│  generated/abi/sim_public_relations.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libsim_public_relations.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── sim_public_relations.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── sim_public_relations.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/sim_public_relations.h
```
...
```c
#include "sim_public_relations.h"

int main() {
    void* handle = sim_public_relations_init();
    if (!handle) return 1;

    int result = sim_public_relations_process(handle, 42);
    if (result != 0) {
        const char* err = sim_public_relations_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    sim_public_relations_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lsim_public_relations -L./zig-out/lib
```

### From Idris2

```idris
import SIM_PUBLIC_RELATIONS.ABI.Foreign
...
```rust
#[link(name = "sim_public_relations")]
extern "C" {
    fn sim_public_relations_init() -> *mut std::ffi::c_void;
    fn sim_public_relations_free(handle: *mut std::ffi::c_void);
    fn sim_public_relations_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = sim_public_relations_init();
        assert!(!handle.is_null());

        let result = sim_public_relations_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libsim_public_relations = "libsim_public_relations"

function init()
    handle = ccall((:sim_public_relations_init, libsim_public_relations), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:sim_public_relations_process, libsim_public_relations), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:sim_public_relations_free, libsim_public_relations), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/sim_public_relations.h
   ```
